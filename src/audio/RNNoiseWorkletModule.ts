...
class RNNoiseWorkletProcessor extends AudioWorkletProcessor {
  ...
  private processRNNoiseFrame(): void {
    ...
    for (let i = 0; i < FRAME_SIZE; i++) {
      this.heapF32[heapIdx + i] = this.inBuf[(this.inR + i) % RING_SIZE] * 32768;
    }
  ...
  public process(inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    for (let i = 0; i < blockSize; i++) {
      this.inBuf[this.inW] = this.mixInputChannels(inputChannels, i, channelCount);
