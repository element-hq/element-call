...
export function createCallViewModel$(
  ...
  isOneOnOneWith$: Behavior<Pick<
    RoomMember,
    "userId" | "getMxcAvatarUrl" | "rawDisplayName"
  > | null>;
  localUserIsAlone$: Behavior<boolean>;
  // sounds and events
  ...
  return {
    autoLeave$: callLifecycle.autoLeave$,
    callPickupState$,
    leave$,
    hangup: () => userHangup$.next(),
    join,
    toggleScreenSharing,
    sharingScreen$,

    tapScreen: () => screenTap$.next(),
    tapControls: () => controlsTap$.next(),
    hoverScreen: () => screenHover$.next(),
    unhoverScreen: () => screenUnhover$.next(),
